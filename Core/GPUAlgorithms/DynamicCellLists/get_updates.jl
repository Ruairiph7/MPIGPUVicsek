...
function get_updates_dcl!(θ_updates, particles, cells_data, cell_list_params, num_particles, numerical_params, min_cell_width, time_step, steps_to_shrink_buffers, ArrayType)
    ...
    if time_step % steps_to_shrink_buffers == 0
        lower_max_num_occupied_cells = lower_max_num_occupied_cells_check(cells_data.num_occupied, cells_data.occupied_coords.IDs, cell_list_params)
    ...
    update_max_num_occupied_cells = check_num_occupied_cells(cells_data.num_occupied,
        cells_data.occupied_coords.IDs, cell_list_params)
