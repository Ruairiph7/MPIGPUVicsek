...
function initialise_data_structures_gppwpn(cell_list_params::CellListParams, N)
    ...
    cell_starts = CuArray(zeros(Int32, cell_list_params.num_cells)) #First index in sorted_particles for cell c
    cell_ends = CuArray(zeros(Int32, cell_list_params.num_cells)) #Last index in sorted_particles for cell c (inclusive)
